...
def SolveHybridStabilized(mesh,orders,stabs,plot=False,export_vtk=False,vtk_str="",remove_cloak=False):
    ...
    if abs(sigma_dic["cloak-inner"]-sigma_dic["host-inner"]) > 1e-12:
        raise Exception('Need to implement integrals for projection stabilization differently as rho_1 != rho_0')
    # this assumes rho_0 = rho_1 to simplify implementation (in not true the exception above will be raised)
    jump_u_Gamma = sigma_dic["cloak-inner"]*grad(u[0]) - sigma_dic["cloak-outer"]*grad(u[1])
    jump_v_Gamma = sigma_dic["cloak-inner"]*grad(v[0]) - sigma_dic["cloak-outer"]*grad(v[1])

    #jump_u_Gamma = {"Gamma-internal":   
    #                "Gamma-outer": sigma_dic["host-inner"]*grad(u[0]) - sigma_dic["cloak-outer"]*grad(u[1])
    #               }
    #jump_v_Gamma = {"Gamma-internal":   
    #                "Gamma-outer": sigma_dic["host-inner"]*grad(v[0]) - sigma_dic["cloak-outer"]*grad(v[1])
    #               }
    ...
    # stabilization of interpolated jump 
    # Step 1: xiG <- Pi_h jump_u_Gamma 
    ...
    #aX += facets_G_indicator *  (xiG - jump_u_Gamma["Gamma-outer"] * nF) * muG * dx( skeleton=True, definedon=mesh.Materials("cloak-outer"))
    # Step 2: Now project using xiG
    #aX += stabs["proj"] * facets_G_indicator * h * (jump_u_Gamma["Gamma-internal"] * nF - xiG) * ( jump_v_Gamma["Gamma-internal"] * nF - muG ) * dx( element_boundary=True, definedon=mesh.Materials("cloak-inner"))
    #aX += stabs["proj"] * facets_G_indicator * h * (jump_u_Gamma["Gamma-outer"] * nF - xiG) * ( jump_v_Gamma["Gamma-outer"] * nF - muG ) * dx( element_boundary=True, definedon=mesh.Materials("cloak-outer"))
    
    #aX += 1e3 * stabs["proj"] * facets_G_indicator * h * (xiG - u[1]) * ( muG - v[1] ) * dx( element_boundary=True, definedon=mesh.Materials("cloak-outer"))
    #aX += 1e3 * stabs["proj"] * facets_G_indicator * h * (xiG - u[0]) * ( muG - v[0] ) * dx( element_boundary=True, definedon=mesh.Materials("cloak-inner"))
    ...
    #fX += facets_G_indicator *  1.0 * muG * dx( skeleton=True)
...
for order in orders_comp: 
    ...
    for maxh in maxhs:
        ...
        mesh = CreateMetamaterialMesh(maxh=maxh,order_geom=order)
