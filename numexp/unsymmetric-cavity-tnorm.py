...
all_maxh = np.linspace(0.4,0.1,3,endpoint=False).tolist() + np.linspace(0.1,0.01,10,endpoint=False).tolist() 

for snd_order_jumps in [True,False]: 
    ...
    for order,stabs in zip([2,3],stabs_order[1:]):
        ...
        for maxh in maxhs:
            ...
            if maxh == maxhs[len(maxhs)-5]:
                result = SolveHybridStabilizedModified(mesh,orders,stabs,problem,True, snd_order_jumps)
            else:
                result = SolveHybridStabilizedModified(mesh,orders,stabs,problem,False, snd_order_jumps)

            hybridstab.append(result[0])
            hybrid_IF.append(result[1])
            tnorm.append(result[2])
            l2err.append(result[3])

        plt.loglog(maxhs,hybridstab,label="stabilized-$H^1$-bulk",marker='+')
        plt.loglog(maxhs,l2err,label="stabilized-$L^2$-bulk",marker='+')
        plt.loglog(maxhs, hybrid_IF,label="stabillized-H^1/2-IF",marker='+')
        plt.loglog(maxhs, tnorm,label="tnorm",marker='+')
        ...
        if snd_order_jumps: 
            ...
            name_str = "Cavity-nonsymmetric-k{0}-unstructured-critical-original.dat".format(order)


        if True:
            results = [maxhnp,  np.array(hybridstab,dtype=float), np.array(hybrid_IF,dtype=float),  np.array(tnorm,dtype=float), np.array(l2err,dtype=float) ]
            header_str = "h H1 IF tnorm L2"
        ...
        if True:
            ...
            if snd_order_jumps: 
                ...
                name_str = "Cavity-nonsymmetric-k{0}-unstructured-critical-log-original.dat".format(order)
